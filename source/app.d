...
");
    ...
    handleSpecJson(specFile, ddoc, expectedFailureExamples);

    foreach (string name; "extensions".dirEntries(SpanMode.depth))
        handleSpecJson(name, ddoc, expectedFailureExamples);
...
`);
    ...
    {
        ddoc.write(`<li id="fail_reason_`, (i+1), `">`, reason, `</li>
`);
    }
    ddoc.write(`</ol>
`);
...
private void handleSpecJson(const string specFile, File ddoc, const JSONValue[string] expectedFailureExamples)
{
    auto specJSON = specFile.readText();
	specJSON = specJSON.strip();
	specJSON.skipOver("[");
	...
	while (specJSON.length > 1)
	{
        ...
        const expectFailure = (exampleNumber in expectedFailureExamples) !is null;
...
private string replaceCodeBlockDelimiters(string markdown, string section)
{
    ...
    void replaceBreak()
    {
        const replacement = breakType == '*' || section.startsWith("Thematic") ? '_' : '*';
        const length = breakEnd - breakStart;
